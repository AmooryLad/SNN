...
from cifar100_detect.backbone import SEWBackboneForDetection
...
class SEWBackboneWithFPN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fpn(feats)          # adds 'p6','p7' via extra_blocks
...
def build_retinanet(
    ...
    - num_classes includes background (VOC = 20 foreground + 1 background).
    - trainable_backbone_layers: 0 freezes all spiking backbone layers; 1/2/3
      progressively unfreezes layer4 / layer3+4 / layer2+3+4. The stem stays
      trainable regardless since it's randomly initialized (new 7x7 conv).
    """
    backbone = SEWBackboneWithFPN(num_steps=num_steps)

    if backbone_ckpt is not None:
        ck = Path(backbone_ckpt)
        if ck.exists():
            backbone.body.load_classifier_weights(str(ck))
        else:
